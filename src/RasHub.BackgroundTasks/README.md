# RasHub Background Task Engine

`RasHub.BackgroundTasks` is the in-process dispatcher for long-running and periodic RasHub work. The module owns
...
The `BackgroundTasks` section controls:
...
`RasHub.BackgroundTasks` and exposes counters for enqueue, deduplication, rejection, start, retry, success, failure, and
cancellation, plus attempt duration. The `background-tasks` readiness check becomes degraded at 80% capacity and
