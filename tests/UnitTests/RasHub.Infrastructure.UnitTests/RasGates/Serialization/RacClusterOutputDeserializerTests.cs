...
namespace RasHub.Infrastructure.UnitTests.RasGates.Serialization;
...
public sealed class RacClusterOutputDeserializerTests
{
    ...
    [Fact]
    public void Deserialize_defaults_fields_absent_in_older_RAC_versions()
    {
        var cluster = Assert.Single(_deserializer.Deserialize(CreateRequiredOutput()));

        Assert.False(cluster.KillByMemoryWithDump);
        Assert.False(cluster.AllowAccessRightAuditEventsRecording);
        Assert.Equal(0, cluster.PingPeriod);
        Assert.Equal(0, cluster.PingTimeout);
        Assert.Equal(string.Empty, cluster.RestartSchedule);
    }
