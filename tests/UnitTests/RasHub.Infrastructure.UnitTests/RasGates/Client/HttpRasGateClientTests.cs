...
public sealed class HttpRasGateClientTests
{
    ...
    [Fact]
    public async Task GetClustersAsync_rejects_unsupported_RAC_version_before_execution()
    {
        var requestCount = 0;
        using var httpClient = CreateHttpClient(_ =>
        {
    ...
    private static HttpRasGateClient CreateClient(
        ...
        string apiKey)
    {
        var deserializer = new RacClusterOutputDeserializer(
            new RacKeyValueOutputDeserializer());
        IRacResourceAdapter<RasClusterSnapshot>[] adapters =
        [
            new RacClusterSnapshotV1Adapter(deserializer),
            new RacClusterInfoV1Adapter(deserializer)
        ];
        ...
        return new HttpRasGateClient(
            ...
            new RacCapabilityResolver(adapters),
