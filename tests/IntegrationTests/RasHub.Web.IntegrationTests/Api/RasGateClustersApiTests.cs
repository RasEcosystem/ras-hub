...
public sealed partial class RasGateClustersApiTests : IClassFixture<RasHubWebApplicationFactory>
{
    ...
    public async Task Refresh_shadow_persists_clusters_and_returns_metadata()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/shadow/refresh",
            null,
        ...
        var stored = Assert.Single(await _factory.FindRasClustersAsync(rasEndpoint.Id));
        ...
        var storedEndpoint = await _factory.FindRasEndpointAsync(
            rasEndpoint.Id);
        Assert.NotNull(storedEndpoint?.LastSeenAt);
        Assert.Equal(
            rasEndpoint.Id,
            _factory.RasGateBoundary.LastRasEndpointId);
        Assert.Equal(rasGate.Id, _factory.RasGateBoundary.LastRasGateId);
        Assert.Equal(
            "ras.example.test:1545",
            _factory.RasGateBoundary.LastRasEndpointAddress);
    }
    ...
    public async Task Refresh_shadow_with_complete_empty_snapshot_removes_last_cluster_and_infobases()
    {
        var rasGate = await _factory.SeedRasGateAsync();
        var rasEndpoint = await _factory.SeedRasEndpointForGateAsync(rasGate);
        var cluster = await _factory.SeedRasClusterAsync(rasEndpoint.Id);
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Empty(await _factory.FindRasClustersAsync(rasEndpoint.Id));
        Assert.True(Assert.Single(
            await _factory.FindRasClustersAsync(rasEndpoint.Id, true)).IsDeleted);
    ...
    public async Task Get_live_all_refreshes_and_returns_complete_shadow()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live/all",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
        ...
        var stored = await _factory.FindRasClustersAsync(rasEndpoint.Id);
    ...
    public async Task Get_shadow_paged_returns_persisted_clusters_without_live_request()
    {
        ...
        using var cachedResponse = await client.GetAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/shadow?page=1&pageSize=10",
            TestContext.Current.CancellationToken);
    ...
    public async Task Get_shadow_all_returns_complete_persisted_collection_without_live_request()
    {
        ...
        using var response = await client.GetAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/shadow/all",
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
        var data = json.GetProperty("data").EnumerateArray().ToArray();
    ...
    public async Task Get_live_paged_refreshes_complete_shadow_and_returns_requested_page()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live?page=2&pageSize=2",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
        var data = json.GetProperty("data");
        ...
        Assert.Equal(
            3,
            (await _factory.FindRasClustersAsync(rasEndpoint.Id)).Count);
    }

    [Fact]
    public async Task Search_shadow_paged_searches_all_endpoints_and_returns_parent_context()
    {
        var firstGate = await _factory.SeedRasGateAsync("First Gate");
        var secondGate = await _factory.SeedRasGateAsync("Second Gate");
        var firstEndpoint = await _factory.SeedRasEndpointForGateAsync(
            firstGate,
            "First RAS");
        var secondEndpoint = await _factory.SeedRasEndpointForGateAsync(
            secondGate,
            "Second RAS");
        ...
        Assert.Collection(
            ...
            item =>
            {
                Assert.Equal(firstEndpoint.Id, item.GetProperty("rasEndpointId").GetGuid());
                Assert.Equal("First RAS", item.GetProperty("rasEndpointName").GetString());
                Assert.Equal(
            ...
            item =>
            {
                Assert.Equal(secondEndpoint.Id, item.GetProperty("rasEndpointId").GetGuid());
                Assert.Equal("Second RAS", item.GetProperty("rasEndpointName").GetString());
                Assert.Equal(
    ...
    [Fact]
    public async Task Search_shadow_all_applies_optional_endpoint_and_selected_host_field()
    {
        var expectedGate = await _factory.SeedRasGateAsync("Expected Gate");
        var otherGate = await _factory.SeedRasGateAsync("Other Gate");
        var expectedEndpoint = await _factory.SeedRasEndpointForGateAsync(
            expectedGate,
            "Expected RAS");
        var otherEndpoint = await _factory.SeedRasEndpointForGateAsync(
            otherGate,
            "Other RAS");
        ...
        using var response = await client.GetAsync(
            $"/api/v1/clusters/shadow/search/all?query=TARGET&rasEndpointId={expectedEndpoint.Id}&fields=Host",
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
        var item = Assert.Single(json.GetProperty("data").EnumerateArray());

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal(expectedEndpoint.Id, item.GetProperty("rasEndpointId").GetGuid());
        Assert.Equal(
    ...
    public async Task Get_live_one_updates_only_requested_cluster()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live/{first.ExternalId}",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
    ...
    public async Task Get_by_id_returns_cached_cluster_without_calling_gate()
    {
        ...
        using var response = await client.GetAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/shadow/{cluster.ExternalId}",
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
    ...
    public async Task Synchronize_by_id_without_info_capability_fails_without_execution()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live/{Guid.NewGuid()}",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
    ...
    public async Task Synchronize_by_id_returns_cluster_sync_error_when_synchronization_fails()
    {
        ...
        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live/{clusterId}",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
    ...
    public async Task Synchronize_by_id_missing_remote_cluster_removes_stale_shadow_and_returns_not_found()
    {
        ...
        var cluster = await _factory.SeedRasClusterAsync(rasEndpoint.Id);
        await _factory.SeedRasInfobaseAsync(cluster.Id);
        var sibling = await _factory.SeedRasClusterAsync(rasEndpoint.Id);
        _factory.RasGateBoundary.ClusterException =
            new RacResourceNotFoundException("clusters", cluster.ExternalId);
        using var client = _factory.CreateAuthenticatedClient();

        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters/live/{cluster.ExternalId}",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
        ...
        Assert.Equal(1, _factory.RasGateBoundary.ClusterInfoRequestCount);
        var activeClusters = await _factory.FindRasClustersAsync(rasEndpoint.Id);
        Assert.Equal(sibling.ExternalId, Assert.Single(activeClusters).ExternalId);
        var allClusters = await _factory.FindRasClustersAsync(rasEndpoint.Id, true);
        ...
        bool singleResource)
    {
        ...
        using var client = _factory.CreateAuthenticatedClient();
        var path = $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters" +
                   (singleResource
                       ? $"/live/{clusterId}"
                       : "/shadow/refresh");
    ...
    [Fact]
    public async Task Update_gate_identity_invalidates_gate_status_but_preserves_endpoint_shadow()
    {
        var rasGate = await _factory.SeedRasGateAsync(
    ...
    [Fact]
    public async Task Synchronize_for_unknown_endpoint_returns_not_found_without_execution()
    {
        using var client = _factory.CreateAuthenticatedClient();

        using var response = await client.PostAsync(
            $"/api/v1/ras-endpoints/{Guid.NewGuid()}/clusters/shadow/refresh",
            null,
            TestContext.Current.CancellationToken);
        var json = await ReadJsonAsync(response);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Cluster_operations_for_inactive_endpoint_return_conflict(
        bool synchronize)
    {
        var rasGate = await _factory.SeedRasGateAsync();
        var rasEndpoint = await _factory.SeedRasEndpointForGateAsync(
            rasGate,
            isActive: false);
        using var client = _factory.CreateAuthenticatedClient();
        var path = $"/api/v1/ras-endpoints/{rasEndpoint.Id}/clusters" +
    ...
    public async Task Synchronize_with_partial_snapshot_keeps_previous_complete_state()
    {
        ...
        var stored = Assert.Single(
            await _factory.FindRasClustersAsync(rasEndpoint.Id));
        ...
        string expectedCode)
    {
        ...
        Assert.Contains("verify the RAS endpoint", message);
        Assert.Contains("assigned RasGate", message);
